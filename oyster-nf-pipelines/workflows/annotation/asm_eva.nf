params.input_fastq = "/home/zl436/palmer_scratch/fastq/rna/*_{1,2}.fastq.gz"
params.dir="/home/zl436/palmer_scratch/oyster_genome"
params.reads_wgs = "${params.dir}/WGS/*_{1,2}_trimmed_grmv.fastq.gz"
...
process TRIM_FASTQ {
	...
	script:
	"""
	fastp -i ${sample}_1.fastq.gz -I ${sample}_2.fastq.gz -o ${sample}_trimmed_1.fastq.gz -O ${sample}_trimmed_2.fastq.gz --trim_front1 15 --trim_front2 15 --cut_mean_quality 20 -l 50
	"""
}
...
process filter_kmer_db{
	...
	script:
	"""
	meryl greater-than 3 ${kmer_db} output ${kmer_db.baseName}_fltr3.meryl
	"""
}
...
process predict_genes{
	container 'quay.io/biocontainers/braker2:2.1.2--0'
	...
	script:
	"""
	braker.pl --genome=${genome} --bam=${merged_bam} --softmasking --cores 8 --species=oyster --working-dir=braker2_output
	"""
...
workflow {
	...
	kmer_fltd_ch=kmer_ch|filter_kmer_db
	kmer_fltd_ch.view()
	...
	genome.view()
        ...
        comb_kmer_ch=genome.combine(kmer_fltd_ch)
        comb_kmer_ch.view()
	...
	//genome_idx_ch=masked_genome|STAR_INDEX
	...
	//predicted_ch=predict_genes(masked_genome,merged_bam)
	//predicted_ch.view()
